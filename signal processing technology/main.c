...
void templateMatchingGray(Image* src, Image* template, Point* position, double* distance)
{
	...
	int x, y, i, j,k;
	int finalcount = 0;
	double cont[4] = {1,2,0.5,0.66666666};
	for(k = 0;k<4;k++){
		...
		for (x = 0; x < src->width - template->width; x++)
		{
			...
			for (j = 0; j < template->height; j++)
			{
				for (i = 0; i < template->width; i++)
				{
					...
					double v;
				    v = (src->data[(y + j)*src->width + (x + i)]*cont[k] - template->data[j*template->width + i]);
					distance += v*v;
					}else{
						count++;
					}
	...
	position->x = ret_x;
	position->y = ret_y;
	*distance = sqrt(min_distance) / (template->width*template->height-finalcount);
}
		...
		for (x = 0; x < src->width - template->width; x++)
		{
			...
			for (j = 0; j < template->height; j++)
			{
				for (i = 0; i < template->width; i++)
				{
					...
					int r = ((src->data[pt + 0])*0.25 - template->data[pt2 + 0]);
					int g = ((src->data[pt + 1])*0.25 - template->data[pt2 + 1]);
					int b = ((src->data[pt + 2])*0.25 - template->data[pt2 + 2]);
...
// test/beach3.ppm template /airgun_women_syufu.ppm 0.0 0.5 cwp
